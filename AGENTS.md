...
Developer CLI and SSOT for 11 CLI-adjacent domains consumed workspace-wide via MRO
(`Toml, Yaml, Csv, Json, Xlsx, Cli, Tui, Run, Dag, Templates, Workflow`). Wraps
typer/click/rich + serialization + templating + workflow/DAG.
...
Domains are nested MRO namespaces under `Cli` (`m.Cli.*`, `u.Cli.*`, …), not 11
top-level dirs.
...
- Consume CLI domains via MRO (`m.Cli.Toml*`, `u.Cli.Toml*`, …) — never fork locally;
  extend the owning domain here.
- Domain-first naming: `yaml_read_files`, `TomlPhaseConfig`, `CSV_DEFAULT_DELIMITER`.
- `u.Cli.render_template` / `config_load` / `yaml_validate_schema` back ADR-005 config
  SSOT.
...
<https://github.com/gastownhall/beads/blob/main/docs/core-concepts/sync-concepts.md> for
